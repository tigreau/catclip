...
18. **Picker previews stay POSIX-free** — any command string handed to fzf (preview commands, `start:reload:` / `change:reload:` bindings, future bind actions) must be a straight pipeline of program invocations with placeholder substitution, never a POSIX shell script. fzf forwards these to `cmd /s /c` on Windows by default, so `set --`, `"$@"`, `if [ -n "$x" ]`, `for v in {+2}`, and shell variable assignments (`name={N}`) all silently break previews under cmd.exe. Push any conditional logic into Go-side `--internal-*` subcommands instead, and verify the placeholders (`{2}`, `{3}`, `{+2}`, `{q}`) can be inlined directly.
    ...
    **Internal launcher boundary:** use `picker.CommandExecutable` for invocation and `picker.CommandArg` for fixed operands, not Go string-literal quoting. Fixed operands must also protect literal fzf placeholder text. PowerShell needs explicit invocation of a quoted executable; cmd executable lookup has a separate encoding boundary from native arguments. `picker.PrepareCommand` supplies the launch context and pins fzf's automatic `SHELL` executor; a nonempty custom `--with-shell` uses different placeholder quoting in pinned fzf. Public replay rendering and direct `exec.Command` argv retain their separate contracts.

    **Raw selection-file exception:** fzf's `f` placeholder flag (`{+f}`) creates an argument-file transport but substitutes its filename without shell-specific `QuoteEntry`. `picker.SelectionFilePlaceholder` supplies the standalone `"{+f}"` exception; ordinary placeholders must not gain extra quotes. fzf runs from a private working/temp directory, and explicitly marked Go helpers restore the original project cwd and temp settings before startup. Relative selection filenames resolve against that private directory, never the restored project. Only the owned private directory is cleaned up after fzf exits. Relative `FZF_DEFAULT_OPTS_FILE` paths retain their original meaning; custom fzf commands run from the private cwd and relative custom history/output paths require compatibility review. **Known Windows limitation:** Windows canonicalizes `TMP=.` to an absolute path, so double quotes still cannot protect shell-special characters in that temp root. Native tests must retain this failing case until a validated solution or explicit deferral; private cwd alone is not a complete Windows fix.

    **Exact live queries:** content command builders keep raw `{q}` for fzf's query-refresh semantics and add an internal marker. The marked Go helper recovers the exact query from `FZF_QUERY`, because legacy PowerShell drops empty native arguments and PowerShell 7 can retain fzf's quote-escape backslashes. This normalization is internal-only and must preserve option-shaped query text literally. Ordinary field placeholders still need independent native tests, including empty fields.
...
19. **Discovery is the parent's job, once per authorized target generation** — the initial interactive target flow enumerates its visible universe once and adopts the confirmed membership as a sealed session generation. An explicit `--no-ignore` or genuinely new target/scope universe may create another generation. Ordinary modifier pickers (`--depth`, `--recent`, `--only`, …), output selection, and final execution consume retained state; they do not run `evaluateScope` to replace same-generation membership. The parent may apply a candidate stage or bounded content/Git work to retained paths and write the result to a temp file. The fzf preview command reads that prediscovered input and formats — nothing more. If a user-visible refresh feature is added later, it must be designed as another explicit generation boundary; v0.7.4 does not ship one.
    ...
    **Shared checkpoint transport is generation-owned, not a body cache.** Large retained states may reference an immutable binary inventory using a versioned JSON descriptor with a generation token, ordered file IDs, and state-local output fields. The reader must preserve order, duplicate IDs, snippet lines, per-entry overrides, Git observations, and ignore policy exactly, rejecting mismatched artifacts rather than rediscovering. Inventory changes invalidate future reuse, never overwrite an old base still needed by undo. The session owns base cleanup independently of state directories. Small/narrow states keep standalone JSON; preview rendering stays asynchronous. See `docs/versions/v0.7.6/reports/ACTIVE_REPORT_shared_checkpoint_transport_2026_09_09.md`.

    **Enforced today:** `TestRunPipelineArchitectureGuards` (`requireInternalRenderHandlersAvoidDerivation`) asserts the per-refresh render handlers — `runInternalPrediscoveredTreePreview`, `runInternalLinesPreview`, `runInternalPrediscoveredContentMatchList`, `runInternalFilePreview`, `runInternalContentCheckpointTreePreview`, `runInternalRecentPreview`, `runInternalSinkPreview`, `runInternalSinkToggle` — never call `evaluateScope` or `discoverInvocation`. Picker drivers and the content-match `change:reload:` path are structurally exempt from that older render-only guard; v0.7.4 must separately prove that, after a generation is sealed, those paths derive from retained candidates rather than enumerate. See `docs/versions/v0.7.4/reports/ACTIVE_PLAN_interactive_single_enumeration_snapshot.md`.

20. **Args describe the command; the sealed generation owns interactive membership** — modifier chaining remains string concatenation, and canonical `currentArgs []string` records what the user chose. Script/direct execution evaluates those args normally. Once an interactive target selection is confirmed, however, the resolved command is paired with an immutable generation containing the exact ordered paths observed by that run. Undo (Esc) restores the argument frame and its associated retained state together; redo-by-selection may reuse a retained child. A cache miss, eviction, checkpoint failure, or sink/final mismatch must rebuild derived artifacts from the generation or return an invariant error, never silently evaluate the args against a newer filesystem view.

    **One interactive run assumes a stable filesystem and uses stable membership plus complete selected-entry metadata.** `Lstat` begins concurrently with target classification, but target confirmation is the barrier: before any filter or output screen opens, every committed selected entry is ready, vanished, or unreadable. There is no bare-command exception. `--size`, `--recent`, previews, reports, and output planning reuse size, modification time, and mode without another primary stat. An explicit `--no-ignore` generation stats only newly admitted paths before it is sealed. Ordinary full/raw bodies are not cached; content filters and final emission open retained paths when their operations run. Files created, deleted, renamed, replaced, chmodded, or modified concurrently are outside the v0.7.4 behavior contract. Existing low-level read errors may still surface naturally, but no mutation outcome authorizes special reconciliation, a watcher, retry loop, or rediscovery.

    Process caches keyed on stable discovery inputs (working directory, `.hiss`, normalized targets) remain valid outside this session contract. Within one interactive session, derived states retain exact ordered membership and stage overlays as immutable children of a sealed parent. Keys include the owning inventory/generation, parent/state identity, ordered candidates, stage semantics, and output projection—not only a modifier value. History required to reconstruct the user's selection is not evictable merely because canonical args exist. Transient materializations and encoded checkpoints may be evicted only when they can be rebuilt without discovery.

    **Memo values are immutable at the ownership boundary, not merely by convention.** Copying an outer Go struct is insufficient when it contains slices or maps: scopes, stages, stage operands, entries, pinned snippet lines, diagnostics, notices, and cached Git observations can still share mutable backing storage with the retained state. Every memo lookup/materialization must return a deep-detached command and discovery view. A caller may mutate any returned collection without changing a later lookup. Output projection is not inventory identity: after projecting retained file IDs back into entries, stamp the current scope's output mode and mode-specific fields (`--lines`, `--snippet`, or diff) before planning or checkpointing. A cached checkpoint must validate the caller's selection and output projection against its retained state; it may tolerate newer cached metadata, but it must not serialize caller-corrupted mode fields. Tests must mutate returned nested fields and must compare retained sink payloads byte-for-byte with canonical discovery.

    **A result-set cache may narrow candidates only with a proof of subset containment.** Textual prefix is a sufficient proof for literal search text (`foo` → `foobar`), but not for regex source (`foo` → `foo|bar` widens the language; `foo` → `foo*` changes the preceding token). Identical patterns may reuse their exact session result. A non-identical prefix extension may restrict to prior matches only when both patterns are provably literal; otherwise it must rescan the complete retained candidate set. That is a bounded content scan over retained paths, never directory discovery. Conservative cache misses are correct. False cache hits that omit files violate rules 1 and 12. See `docs/versions/v0.7.4/ACTIVE_NOTE_retained_picker_cache_correctness.md`.
...
22. **Resolved command parity** — every command Catclip prints as "Resolved command:" must describe the same targets, stages, ordering, and output choices the user committed. On a stable filesystem, copy-pasting it into a new non-interactive invocation must produce the same selection. Startup pickers, modifier pickers, sink picker, and argv normalizations (e.g., dynamic pattern inference collapsing repeated literals into wildcards, redundant-literal removal) all participate in building this canonical argv; none may rely on hidden picker-frame state, RNG, or in-process side effects to change command meaning. If a normalization rewrites argv, the rewritten form must be a true equivalent under the same filesystem observation — losing or gaining a file is a bug, not a normalization.

    Interactive execution additionally obeys rule 20's session guarantee: after targets are confirmed, output/finalization consumes the sealed paths paired with that command rather than rerunning it. A future replay may legitimately observe files added or removed after the interactive snapshot; compact argv cannot preserve historical membership, and persistent membership checkpoints are not promised. Shell rendering remains part of the command contract: user-facing commands target POSIX shells on macOS/Linux/WSL and PowerShell on Windows, including literal handling of spaces, `$`, backticks, backslashes, glob characters, and apostrophes. Internal fzf preview commands follow their separate subprocess-shell contract.
...
26. **Never open an output sink picker for an empty plan** — the sink picker asks where usable output should go and previews that output. Once the prepared `output.Plan` is empty, every sink choice is meaningless and the preview pane is necessarily blank. Both startup sink entry points (`maybeResolveStartupSinkPickerArgs` and undo-aware `runStartupSinkFrame`) must carry the prepared discovery/plan forward without invoking fzf; normal execution then owns the resolved-command header, precise discovery diagnostics, generic no-files fallback when still applicable, and exit code. Do not turn this gate into an early custom error: that would bypass the shared diagnostic policy and make interactive empty results differ from replaying the resolved command.
...
- Positional target grammar has one metacharacter boundary: no glob metacharacter means exact/fuzzy file-and-directory navigation; any `*`, `?`, or `[` means deterministic file-glob selection. Wrapper stars are not fuzzy punctuation: `catclip "*util*"` must reach the glob resolver unchanged, while `catclip util` is the fuzzy navigation form. Glob targets never open fzf on zero, one, or many matches, and interactive/headless runs must select the same file set.
- Shell quoting preserves one argv value; it does not override Catclip's target grammar. A literal filename containing `*`, `?`, or `[` is still classified as a target glob. A root filename that begins like a public option must retain an explicit path prefix such as `./--paths`; picker-generated commands must not emit a bare option-shaped target. The current picker replay defect is tracked in `docs/versions/v0.6.8/reports/ACTIVE_BUG_option_shaped_root_target_replay.md`.
...
- Path-shaped target globs preserve structural depth: `catclip "src/*.tsx"` matches direct TSX children of `src/` because target `*` does not cross `/`. Positional target patterns containing `**` are rejected rather than retaining Go `path.Match`'s misleading one-component behavior.
- Positional glob targets are matched against discovered file basenames and cwd-relative file paths, never directory entries. A glob ending in `/` such as `catclip "internal/*/"` therefore matches nothing even when suitable directories exist. Use the exact directory target (`catclip internal`) for recursive traversal. Never print a recursive recovery command using target `**`; the corrected v0.6.7 diagnostic is recorded in `RESOLVED_BUG_recursive_glob_hint_uses_unsupported_doublestar.md`.
- Real target globstar is not a priority. Exact basenames (`Button.tsx`) and parent-qualified suffixes (`components/Button.tsx`) already resolve anywhere with Catclip's ambiguity gate, and recursive typed selection is already expressed as `catclip src --only "*.tsx"`. Assigning recursive meaning to the reserved target `**` would mostly duplicate target discovery.
- `--only` and `--exclude` deliberately remain recursive post-target filters. Their legacy stage glob `*` may cross `/`, so `catclip . --only "src/*.tsx"` includes nested TSX files below `src/`, unlike the same string used as a positional target. Adjacent `**` is rejected in direct CLI filter values because it adds no expressive power in this matcher and falsely suggests ripgrep globstar semantics. Exact paths read by `--only -` / `--exclude -` are identities and remain exempt. Preserve the single-star compatibility unless a future version deliberately redesigns the matcher; see `docs/versions/v0.6.8/reports/RESOLVED_PLAN_reject_cli_double_star.md`.
- Trailing-slash filter syntax is literal-only: `--only "handler/"` and `--exclude "handler/"` select a directory subtree, but adding any glob metacharacter changes matcher class first. `--only "*/"` is a file-path glob ending in `/` and matches nothing. Reached dead glob values must warn, and an all-dead `--only` stage owns the resulting empty state instead of printing speculative generic causes. Do not recommend glob-shaped trailing-slash spellings.
...
- The interactive modifier menu can apply no-value `--no-ignore` or collect
  ...
  modifier stage.
- `--headless` is the explicit no-prompt switch: it forbids interactive pickers (startup recovery, target picker, modifier value pickers), but it does not disable non-interactive fuzzy target lookup. Agents and scripts pass it to guarantee that no picker opens; the bundled fzf binary may still run in filter mode to rank candidates. Exact paths and exact basename resolution continue to win before fuzzy fallback. After exact resolution misses, fuzzy fallback must send the same complete, mixed file-and-directory rows used by the interactive target picker through one fzf filter pass. Use fzf's complete ranked result: a matching directory and matching files below it remain separate candidates because that is what the picker shows. One result auto-resolves, while several return a ranked ambiguity error that lists candidates so the caller can refine the command. Preserve file-only and directory-only errors when every result has one kind; use `[file]`/`[dir]` labels when the result is mixed. No fuzzy branch may accept its lone match before both candidate kinds have been considered. `-q` is independent and does not by itself disable prompts. The resolved headless fall-through defect is recorded in `docs/versions/v0.6.8/reports/RESOLVED_BUG_headless_fuzzy_dir_shadowing.md`.
