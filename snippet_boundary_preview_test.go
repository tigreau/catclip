...
func snippetBoundaryCommittedOutput(view resolvedScopeView, pattern string, choice startupSnippetBoundaryChoice, matched []fileEntry) (string, error) {
	scope := snippetBoundaryPreviewScope(view.Scope, pattern, choice)
	entries := append([]fileEntry(nil), matched...)
	...
	plan, err := buildOutputPlanForResolvedScopes(view.GitContext, []executionScope{scope}, evaluatedScopes, entries)
	if err != nil {
		return "", err
	}
	var buf bytes.Buffer
	if err := writeOutputPlanPayloadWithoutPrefetch(&buf, emitConfig{}, plan); err != nil {
		return "", err
	}
...
func streamSnippetBoundaryToString(tb testing.TB, view resolvedScopeView, pattern string, choice startupSnippetBoundaryChoice, matched []fileEntry) string {
	tb.Helper()
...
func benchSnippetBoundaryView(tb testing.TB, n int) resolvedScopeView {
	...
	return resolvedScopeView{
		Invocation: invocationConfig{WorkingDir: project},
