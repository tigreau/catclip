...
func validateCurrentScopeStageSequence(currentArgs []string, additions []scopeStageKind) error {
...
func buildScopeStageTransitionState(stages []scopeStage) (scopeStageTransitionState, error) {
...
func (s *scopeStageTransitionState) apply(kind scopeStageKind) error {
...
func validateOutputModeTransition(current scopeStageKind, next scopeStageKind, nextSemantics scopeStageSemantics) error {
	...
	if (isDiffOutputStage(current) && next == scopeStageSnippet) ||
		(current == scopeStageSnippet && isDiffOutputStage(next)) {
		return diffSnippetConflictError()
	}
	return outputModeConflictError(scopeStageFlagLabel(current), scopeStageFlagLabel(next))
}

func isDiffOutputStage(kind scopeStageKind) bool {
	switch kind {
...
func outputModeConflictError(currentFlag, nextFlag string) error {
	return validationFailure{
		Reason:       validationReasonOutputModeConflict,
...
func currentScopeStagesFromArgs(args []string) []scopeStage {
...
func currentScopeStagesFromCommandSpec(args []string) ([]scopeStage, bool) {
...
func currentScopeStagesFromArgsLegacy(args []string) []scopeStage {
	stages := make([]scopeStage, 0, 8)
	for i := 0; i < len(args); i++ {
		...
		case "--include", "--only", "--exclude":
			...
			i = next - 1
		case "--recent":
			stages = append(stages, scopeStage{Kind: scopeStageRecent})
...
func scopeStageKindsForFlags(flags []string) ([]scopeStageKind, error) {
	kinds := make([]scopeStageKind, 0, len(flags))
...
func scopeStageKindForFlag(flag string) (scopeStageKind, bool) {
...
func scopeStageFlagLabel(kind scopeStageKind) string {
	semantics, ok := scopeStageSemanticsForKind(kind)
...
func scopeStageSemanticsForKind(kind scopeStageKind) (scopeStageSemantics, bool) {
...
func diffSnippetConflictError() error {
	return validationFailure{Reason: validationReasonDiffSnippetConflict}
}

func diffContentFilterOrderError(flag, boundaryFlag string) error {
	return validationFailure{Reason: validationReasonDiffContentFilterOrder, Flag: flag, BoundaryFlag: boundaryFlag}
}

func diffGitFilterOrderError(flag, boundaryFlag string) error {
	return validationFailure{Reason: validationReasonDiffGitFilterOrder, Flag: flag, BoundaryFlag: boundaryFlag}
}

func snippetContentFilterOrderError(flag string) error {
	return validationFailure{Reason: validationReasonSnippetContentFilterOrder, Flag: flag}
}

func repeatedOutputModeError(flag string) error {
	return validationFailure{Reason: validationReasonRepeatedOutputMode, Flag: flag}
}
