...
func TestRenderSinkPreviewMatchesEmitShapeWithHighlighting(t *testing.T) {
	...
	if err := writeOutputPlanPayloadWithoutPrefetch(&want, emitConfig{}, plan); err != nil {
		t.Fatalf("writeOutputPlanPayloadWithoutPrefetch returned error: %v", err)
	}
	preview, err := renderSinkOutputTextPreview(plan, emitConfig{}, int64(want.Len()+100))
	if err != nil {
...
func TestRenderSinkPreviewTruncatesAtLimit(t *testing.T) {
	...
	preview, err := renderSinkOutputTextPreview(plan, emitConfig{}, 32)
	if err != nil {
		t.Fatalf("renderSinkOutputTextPreview returned error: %v", err)
	}
	if !preview.Truncated {
...
`)
	...
	if len(result.PreparedOutput.Plan.items) != 1 {
		t.Fatalf("expected prepared plan with 1 item, got %d", len(result.PreparedOutput.Plan.items))
	}
}
...
func TestSinkPreviewTogglePersistsAcrossPreviewReruns(t *testing.T) {
	...
	plan := testSinkOutputPlan(t, project, "src/main.go")
	report, err := buildOutputReportForPlan(renderConfig{Preview: true}, gitContext{}, plan, nil)
	if err != nil {
	...
	if err != nil {
		t.Fatalf("prepareStartupSinkPreviewFiles returned error: %v", err)
	}
	defer files.Cleanup()

	first := runSinkPreviewCommand(t, files.PreviewCommand)
	if strings.Contains(first, "[preview mode: tree/report") {
		t.Fatalf("expected output-text mode first, got %q", first)
	}

	toggle := sinkPreviewToggleCommandForTest(t, files.ToggleBinding)
	runShellCommand(t, toggle)

	second := runSinkPreviewCommand(t, files.PreviewCommand)
	if !strings.Contains(second, "[preview mode: tree/report") {
		t.Fatalf("expected tree/report mode after toggle, got %q", second)
	}

	third := runSinkPreviewCommand(t, files.PreviewCommand)
...
func testSinkOutputPlan(t *testing.T, project, relPath string) outputPlan {
	t.Helper()
	...
	plan, err := prepareOutputPlan(gitContext{}, []fileEntry{{
		AbsPath:    absPath,
		RelPath:    relPath,
		Mode:       entryModeFull,
		SizeBytes:  info.Size(),
	...
	if err != nil {
		t.Fatalf("prepareOutputPlan returned error: %v", err)
	}
	if len(plan.items) == 0 {
...
func TestMeasureOutputForSinkMenuUsesLargeMenuAtBundleThreshold(t *testing.T) {
	project := setupTestProject(t, map[string]string{
		"src/large.txt": strings.Repeat("x", bundleThreshold),
	})
	plan := testSinkOutputPlan(t, project, "src/large.txt")

	measurement := measureOutputForSinkMenu(plan, emitConfig{})
	...
	if measurement.Bytes < bundleThreshold {
		t.Fatalf("expected measured bytes >= %d, got %d", bundleThreshold, measurement.Bytes)
	}
}
