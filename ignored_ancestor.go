...
func targetNotFoundOrIgnoredAncestorMessage(r *scopeResolver, target string, scopeIndex int, colors colorPalette) string {
...
func (r *scopeResolver) findIgnoredAncestors(target string) []ignoredAncestorCandidate {
...
func (r *scopeResolver) findBlockerForPath(rel string, isDir bool) *ignoredAncestorCandidate {
...
func ignoredAncestorMessage(target string, scopeIndex int, candidates []ignoredAncestorCandidate, colors colorPalette) string {
	if len(candidates) == 0 {
		return ""
	}
	const maxShown = 5
