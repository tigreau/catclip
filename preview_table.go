...
func renderPreviewTable(cfg renderConfig, gitCtx gitContext, plan outputPlan, report outputReport, stdout, stderr io.Writer, workingDir string, now time.Time, colors colorPalette) error {
	if !cfg.Quiet {
...
// times are guaranteed via ensureEntryModTimes (it stats only entries that lack
// one). Size/git/mode come from the already-built report so the table agrees with
// the tree and the copied payload.
func buildPreviewTableRows(plan outputPlan, report outputReport, workingDir string, now time.Time) ([]previewTableRow, error) {
	seen := make(map[string]bool, len(plan.items))
	order := make([]fileEntry, 0, len(plan.items))
	for _, item := range plan.items {
		if seen[item.relPath] {
			continue
		}
		seen[item.relPath] = true
		order = append(order, item.entry)
	}

	withTimes, err := ensureEntryModTimes(order, workingDir)
	if err != nil {
		return nil, err
	}
	...
	for _, e := range withTimes {
		...
		humanSize, tokens := treepkg.FormatSizeAndTokens(size, 1)
...
func writePreviewTable(w io.Writer, rows []previewTableRow, colors colorPalette) error {
...
// alignment.
func writePreviewTableAligned(w io.Writer, rows []previewTableRow, colors colorPalette) error {
...
func padColoredCell(plain, color string, width int, colors colorPalette) string {
...
func colorPreviewPath(path string, colors colorPalette) string {
...
func previewSizeColor(colors colorPalette, sizeBytes int64) string {
	switch {
...
func previewRecencyColor(colors colorPalette, b recencyBucket) string {
	switch b {
...
func previewGitColor(colors colorPalette, status string) string {
	switch status {
	case "M", "SM":
		return colors.Warn
	case "S":
...
func previewShapeColor(colors colorPalette, shape string) string {
	if shape == string(entryModeFull) {
		return colors.Dim
	}
	return colors.Git
}
