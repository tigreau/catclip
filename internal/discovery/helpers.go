...
type ScopeView struct {
	...
	Entries    []Entry
	// Targets is the current scope's positional targets. The content-match
	// checkpoint retains them so the child's scope matches the parent's — without them the
...
// SetScopeViewResolver — content picker setup reports an error in that case
// instead of spawning a rediscovering child with unbounded arguments.
