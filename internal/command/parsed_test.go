...
func TestParsedIsInternalKindCoversEveryInternalField(t *testing.T) {
	cases := []struct {
		...
		{name: "TargetSelectionPath", cfg: Parsed{TargetSelectionPath: "/tmp/selection.tsv"}},
		{name: "TargetRootsPath", cfg: Parsed{TargetRootsPath: "/tmp/roots.json"}},
		{name: "CheckpointScope", cfg: Parsed{CheckpointScope: true}},
		{name: "DiffPreviewStatePath", cfg: Parsed{DiffPreviewStatePath: "/tmp/diff.json"}},
