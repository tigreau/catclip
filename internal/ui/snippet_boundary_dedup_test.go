...
// match set — discovery.FilterEntriesByContent over the already-resolved view entries —
...
func TestSnippetBoundaryMatchSetEquivalence(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("discovery.FilterEntriesByContent: %v", err)
	}
	...
	if len(aPaths) != len(bPaths) {
		t.Fatalf("match-set size differs: discovery.FilterEntriesByContent=%d (%v) contentMatchPathsForArgs=%d (%v)", len(aPaths), aPaths, len(bPaths), bPaths)
	}
	for i := range aPaths {
	...
	for _, n := range []int{1000, 4000} {
		...
		b.Run("n="+strconv.Itoa(n)+"/before_3scans", func(b *testing.B) {
			for range b.N {
				...
				if _, err := discovery.FilterEntriesByContent(discovery.EnsureEntryAbsPaths(view.Entries, view.Invocation.WorkingDir), pattern); err != nil {
					b.Fatal(err)
				}
				if _, err := contentMatchPathsForArgs(currentArgs, "--snippet", pattern); err != nil {
					b.Fatal(err)
