...
func TestMaxLinesForFilesEmpty(t *testing.T) {
	got, err := search.MaxLinesForFiles(nil)
	if err != nil {
		t.Fatalf("search.MaxLinesForFiles(nil) returned error: %v", err)
	}
	if got != 0 {
...
func TestMaxLinesForFiles(t *testing.T) {
	...
	got, err := search.MaxLinesForFiles(abs)
	if err != nil {
...
func TestMaxLinesForSizedEntriesIsExactAndPreservesOrder(t *testing.T) {
	...
	entries := make([]discovery.Entry, 0, len(files))
	for _, f := range files {
...
func benchmarkCatclipTestDataEntries(tb testing.TB) []discovery.Entry {
	tb.Helper()
	...
	rels := splitTestNullSeparated(raw)
	entries := make([]discovery.Entry, 0, len(rels))
...
func TestResolveStartupLinesArgsCancelStart(t *testing.T) {
	...
	if err.Error() != discovery.ErrSelectionCancelled.Error() {
		t.Fatalf("expected discovery.ErrSelectionCancelled, got %v", err)
	}
}
...
// splitTestNullSeparated duplicates the search-package helper privately so
// the maxLines benchmark can drive the rg --files harness without exporting
// a helper whose only caller is this one test.
func splitTestNullSeparated(data []byte) []string {
	if len(data) == 0 {
		return nil
	}
