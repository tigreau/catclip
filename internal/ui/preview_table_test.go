...
// discovery.EnsureEntryModTimes does not stat real files) for the table renderer tests.
func previewTableFixture() (output.Plan, output.Report, time.Time) {
	now := time.Date(2026, 5, 27, 15, 45, 0, 0, time.UTC)
	items := []output.PlanItem{
		output.NewFilePlanItem(output.PreparedFileUnit{Entry: discovery.Entry{RelPath: "src/a.go", AbsPath: "/x/src/a.go", ModTime: time.Date(2026, 5, 27, 9, 30, 0, 0, time.UTC), Mode: command.EntryModeFull}}),
		output.NewFilePlanItem(output.PreparedFileUnit{Entry: discovery.Entry{RelPath: "docs/very/long/path/notes.md", AbsPath: "/x/docs/very/long/path/notes.md", ModTime: time.Date(2026, 3, 11, 2, 17, 0, 0, time.UTC), Mode: command.EntryModeFull}}),
	}
	plan := output.PlanFromItemsForTesting(items)
...
func TestPreviewTablePlainParseable(t *testing.T) {
	...
	if err := RenderPreviewTable(RenderConfig{Preview: true, Quiet: true}, git.Context{}, plan, report, &buf, io.Discard, "", now, platform.Palette{}); err != nil {
...
func TestPreviewTableNormalAligned(t *testing.T) {
	plan, report, now := previewTableFixture()
	colors := platform.Palette{Dir: "<dir>", OK: "<ok>", Warn: "<w>", Label: "<lbl>", Dim: "<dim>", Git: "<git>", Reset: "<r>"}
	var buf bytes.Buffer
	if err := RenderPreviewTable(RenderConfig{Preview: true, Quiet: true}, git.Context{}, plan, report, &buf, io.Discard, "", now, colors); err != nil {
		t.Fatalf("RenderPreviewTable: %v", err)
	}
	out := buf.String()
...
func TestPreviewSizeColorMatchesTree(t *testing.T) {
	colors := platform.Palette{Dim: "<dim>", Warn: "<w>", Err: "<e>"}
	cases := []struct {
		bytes int64
		want  string
	}{
...
func TestPreviewTableRowsAreEmissionOrderUnique(t *testing.T) {
	...
	plan := output.PlanFromItemsForTesting([]output.PlanItem{
		output.NewFilePlanItem(output.PreparedFileUnit{Entry: discovery.Entry{RelPath: "z.go", AbsPath: "/x/z.go", ModTime: mt, Mode: command.EntryModeSnippet}}),
		output.NewFilePlanItem(output.PreparedFileUnit{Entry: discovery.Entry{RelPath: "z.go", AbsPath: "/x/z.go", ModTime: mt, Mode: command.EntryModeSnippet}}), // second range, same file
		output.NewFilePlanItem(output.PreparedFileUnit{Entry: discovery.Entry{RelPath: "a.go", AbsPath: "/x/a.go", ModTime: mt, Mode: command.EntryModeFull}}),
	})
	report := output.Report{Sizes: map[string]int64{"z.go": 200, "a.go": 100}}
