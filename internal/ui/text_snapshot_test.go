...
func TestLoadTextSnapshotPreservesRawBytesAndCanonicalPreviewText(t *testing.T) {
	...
	snapshot, err := output.LoadTextSnapshot(absPath, "raw.txt")
	if err != nil {
	...
	wantLines := output.SplitLogicalLines(raw)
...
func TestSnippetResolutionMatchesPreviewAndPreparedPayload(t *testing.T) {
	...
	matchedLines, err := search.RunRipgrepMatchLines("TODO", []string{absPath})
	if err != nil {
		t.Fatalf("runRipgrepMatchLines returned error: %v", err)
	}
	snippet, err := output.ResolveSnippetFromSnapshot(snapshot, matchedLines[absPath], output.SnippetOptions{Mode: output.SnippetBoundaryBlock})
	if err != nil {
		t.Fatalf("output.ResolveSnippetFromSnapshot returned error: %v", err)
	}
	wantRanges := []output.SnippetRange{{Start: 1, End: 4}, {Start: 6, End: 8}}
	if got := snippet.Ranges; !reflect.DeepEqual(got, wantRanges) {
		t.Fatalf("Ranges = %#v, want %#v", got, wantRanges)
	}

	doc, ok := buildInternalSnippetPreviewDocument(relPath, absPath, "TODO", output.SnippetOptions{Mode: output.SnippetBoundaryBlock})
	if !ok {
	...
	payload, _, ranges, err := output.BuildPreparedSnippetPayload(discovery.Entry{
		AbsPath:        absPath,
		RelPath:        relPath,
	...
	if err != nil {
		t.Fatalf("output.BuildPreparedSnippetPayload returned error: %v", err)
	}
